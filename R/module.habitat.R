...
### module parameters
#   - export params to global list `abm.globals$params`;
abm.globals$params[['MODULE.NAME']] <<- list()

#   - instantiate data structure for module stats
...
habitat.react.event = function(event.time, event.type)
    if (event.type=="init") {
        # do stuff for this event

        
        # schedule the next event
        schedule.event(TIME, "MODULE.NAME", "EVENT.TYPE")
    } else {
        # do stuff for this event
