...
SpreadEvents<-
   ...
   # should sanity check map extents
    ...
    n<-1
#ben = microbenchmark(
    Spreads = new("RasterLayerSparse",extent = extent(Landscape))    
    dim(Spreads)<-c(96,96,1)
#    Spreads<- setValues(raster(Landscape),0)
#     Spreads<- raster(Landscape)
#     Spreads[] <- 0
#    Spreads[Loci]<-n
    Spreads <- setValues(Spreads,index = Loci,rep(n,length(Loci)))
#)
...
#      Potentials<-Potentials[ifelse(is.na(tmp),FALSE,tmp==0,]
      Potentials<-Potentials[is.na(tmp),]
      ...
      Spreads[Events] <- n #<- setValues(Spreads,index = Events,rep(n,length(Events)))
...
#      plot(Spreads)
    ...
    if(length(Loci)>500) stop("LEngth Loci GT 500")
    }
    return(Spreads)
}