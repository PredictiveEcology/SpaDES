...
setMethod(
  ...
  definition = function(sim, params, objects, objFn, cl, optimizer,
    ...
    if (optimizer == "DEoptim") {
      ...
      output <- do.call("DEoptim2", deoptimArgs)
...
################################################################################
#' Replacement of DEoptim function, allowing custom parallel cluster
#'
#' This is a verbatim copy of \code{\link[DEoptim]{DEoptim}}, but with
          ...
          cl, fnMap = NULL)
{
  ...
  if (ctrl$parallelType == 3) {
    if (missing(cl))
      stop("If parallelType is 3, then you must provide a cluster to cl arg")
