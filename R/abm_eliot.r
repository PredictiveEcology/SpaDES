...
# SELES scenario 1
    ...
    al = AgentLocation(Which(hab > 2.2 ))
    pri = ProbInit(hab,al)
    na = NumAgents(N)
    # Make agents
    caribou = new("mobileAgent", agentlocation=al, probinit = pri, numagents = na)
    plot(hab)
    points(caribou,pch=19,cex = 0.1)
    for(i in 1:ntimesteps) {
      ex =  hab[caribou@position] # found out what pixels the individuals are on now
      wh = which(!is.na(ex))
    ...
    #  This is in the spirit of Sarah's model
      rads = sample(10:30,length(caribou),replace = T)
...
# Scenario 2 = a specific number of agents to start for specific patches
  ...
  plot(patches)
  points(caribou, col=extract(patches, caribou@position),pch=19,cex = 0.3)
...
dfp = distanceFromPoints(hab,caribou@position)
