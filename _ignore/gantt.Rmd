...
sim2gantt <- function(sim) {
  ...
  mult <- if (ts=="second") {
    as.numeric(dsecond(1)) / as.numeric(dweek(1))
  } else if (ts=="hour") {
    as.numeric(dhour(1)) / as.numeric(dweek(1))
  } else if (ts=="day") {
    as.numeric(dday(1)) / as.numeric(dweek(1))
  } else if (ts=="week") {
    as.numeric(dweek(1)) / as.numeric(dweek(1))
  } else if (ts=="month") {
    as.numeric(dmonth(1)) / as.numeric(dweek(1))
  } else if (ts=="year") {
    as.numeric(dyear(1)) / as.numeric(dweek(1))
  }
  out <- lapply(unique(dt$moduleName), function(x) {
    data.frame(task = dt[moduleName==x]$eventType,
                   ...
                   start = paste0(dt[moduleName==x]$eventTime * mult, strtrim(ts, 1)),
                   end = paste0((dt[moduleName==x]$eventTime + 3)*mult, strtrim(ts, 1)))
  })
