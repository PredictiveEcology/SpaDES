...
event driven: an activity changes the state of the system at particluar times. a particular activity may have several events associated with it. future events are scheduled in an event queue, and then processed in order.
...
<<using-SpaDES, eval=TRUE, echo=TRUE, fig=TRUE>>=
library("SpaDES")
...
<<load-landscape-maps, echo=TRUE, eval=FALSE>>=
...
<<fire, eval=TRUE, echo=TRUE, fig=FALSE>>=
nFires <- 10
landscape[["Fires"]] <-
...
\paragraph{}
Analogously, possible to use \texttt{SpatialPolygons*}, but we don't have \texttt{simPlot} plotting methods yet.
...
meanPixelsBurned1 <- structure(list(mapSize = c(100, 316, 1000, 3162, 10000, 100,
