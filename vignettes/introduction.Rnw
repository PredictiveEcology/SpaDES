...
\paragraph{}
You can inspect the contents of a \texttt{simList} object as you would any other \textsf{R} object (e.g., \texttt{show(mySim)}).
...
\paragraph{}
To model mobile point agents, e.g., animals (as opposed to non-mobile agents such as plants), use a \texttt{SpatialPointsDataFrameNamed} containing additional columns for storing agents' previous \texttt{n} positions.
