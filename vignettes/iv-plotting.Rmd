...
vignette: >
  ...
  %\VignetteIndexEntry{04 Plotting with SpaDES}
...
## Layer types
Its main purpose is to plot spatial type objects. Specifically, it currently can plot `RasterLayer`, `RasterStack`, `SpatialPoints*`, and `SpatialPolygons*` objects. Because `Plot` uses the `grid` package, changing plot parameters is with the `gp=gpar()` designation. See `?gpar` for options.
...
can also be used to redraw everything after a window gets resized manually and objects become
distorted. 
