...
vignette: >
  ...
  %\VignetteDepends{archivist, igraph, RColorBrewer, SpaDES}
...
This functionality can be achieved within an experiment call. This can be done 2 ways, either "internally" through the cache argument. This will have the effect of caching *each spades call*, or "externally" which will *cache the entire experiment*. If there are lots of `spades` calls, then the former will be slow as the `simList` will be digested once per spades call.

### Using cache argument
```{r experiment-cache}
system.time(sims1 <- experiment(mySim, replicates = 2, cache = TRUE, .plotInitialTime=NA))
# internal -- 2nd time faster
...
Notice that speed up can be enormous; in this case ~100x. 
