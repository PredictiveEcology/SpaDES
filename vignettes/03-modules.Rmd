...
A `SpaDES` module consists of a single `.R` source file, containing the code for:
...
parameters <- list(.globals=list(.stackName="landscape", .outputPath=outputPath,
                   ...
                   fireSpread=list(nFires= 10L, .plotInitialTime=0.1, .plotInterval=10),
                   caribouMovement=list(.plotInitialTime=1.01, .plotInterval=1)
                   )
...
Default parameters defined in each module using `defineParameter`.

The parameter list in the `simList` object may be used to pass parameter values to modules. The nested named list structure allows passing as many parameters as needed for your simulation. We suggest passing a list of all the parameters needed for a single module together.

\newpage

# Core `SpaDES` modules

There are a number of built-in modules that provide useful functionality. These modules have their names prefaced with a dot.
...
parameters <- list(.globals=list(.stackName="landscape", .outputPath=outputPath),
                   ...
                   .progress=list(graphical=NA, interval=NA),
...
3. check module compatibility (*e.g.*, spatial extent, timeframe, etc.) (`simInit` does a basic version of this).
