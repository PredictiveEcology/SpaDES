...
**`timestep`**        | Describes the length of time (in seconds) corresponding to 1.0 simulation time units.
...
require(igraph) # required for plotting dependencies
require(SpaDES)
...
plot(depsGraph(mySim, FALSE))       # visual representation of all dependencies
...
There are a number of built-in modules that provide useful functionality. These modules have their names prefaced with a dot, and are summarized in the following table and described further below.

**Module Name**   | **Description** | **Parameterization**
------------------|-----------------|-------------------------------------------
**`.checkpoint`** | brief summary   | supplied to `simInit` per regular module:
&nbsp;            |                 | `.checkpoint$file`
&nbsp;            |                 | `.checkpoint$interval`
**`.load`**       | brief summary   | supplied to `simInit` per regular module:
&nbsp;            |                 | `.load$fileList`
**`.plot`**       | brief summary   | supplied to `simInit` via another module:
&nbsp;            |                 | `moduleName$.plotInitialTime`
&nbsp;            |                 | `moduleName$.plotInterval`
**`.progress`**   | brief summary   | supplied to `simInit` per regular module:
&nbsp;            |                 | `.progress$graphical`
&nbsp;            |                 | `.progress$interval`
**`.save`**       | brief summary   | supplied to `simInit` via another module:
&nbsp;            |                 | `moduleName$.saveInitialTime`
&nbsp;            |                 | `moduleName$.saveInterval`
&nbsp;            |                 | `moduleName$.saveObjects`
&nbsp;            |                 | `moduleName$.savePath`
...
```{r load-save, echo=TRUE, eval=FALSE, message=FALSE}

### WARNING this simInit code doesn't work...
...
mySim <- simInit(times=list(start=0.0, stop=100),
                 params=list(
                   randomLandscapes=list(nx=100L, ny=100L,
...
All contributed modules should be well documented. In addition to providing *complete* metadata within the module's `.R` file, the following additional components should be included:
