...
Plot(caribou, new=T, gpAxis=gpar(cex=0.4), size=1)
...
with the `new=T` argument within the call to `Plot`.
...
# `ggplot2` and `histogram` objects
It can handle `ggplot2` objects or base histogram objects via call to `exHist <- hist(1:10, plot=F)`, but these non-spatial objects cannot be mixed among types (i.e., can't mix and match spatial and non-spatial objects, or base histogram and ggplot2 types). Customization of the ggplot2 elements can be done as a normal ggplot2 plot, then added with `Plot(ggplotObject)`. Note that using ggplot2 means that all plot modifications must be done via the ggplot2 mechanisms, not Plot mechanisms.
```{r ggplotObjects, eval=TRUE, echo=TRUE, cache=TRUE}
ggObj <- ggplot2::qplot(rnorm(1e3))
