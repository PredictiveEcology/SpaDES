...
You can inspect the contents of a `simList` object as you would any other `R` object (*e.g.*, `show(mySim)`). Simulation module object dependencies can be viewed using:

```{r view-dependencies, eval=FALSE, echo=TRUE}
library(igraph)
Plot(depsGraph(mySim, TRUE)) # plots simplified dependency graph
depsEdgeList(mySim, FALSE)   # data.frame of all object dependencies
```

function, or viewed directly by printing the output of the `depsEdgeList` function.
