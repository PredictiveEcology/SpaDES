...
+         simPlot(get(simGlobals(sim)$.stackName, envir = .GlobalEnv))
...
+         simPlot(get(simGlobals(sim)$.stackName), col = .cols[c(4, 
+             7, 10, 3, 8, 5)])
+         sim <- scheduleEvent(sim, simCurrentTime(sim) + simParams(sim)$fireSpread$.plotInterval, 
+             "fireSpread", "plot")
+     }
+     else if (eventType == "plot") {
+         simPlot(get(simGlobals(sim)$.stackName)$Fires, add = TRUE, 
...
+     assign(simGlobals(sim)$.stackName, stack(landscapes, Fires), 
+         envir = .GlobalEnv)
