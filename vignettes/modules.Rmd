...
A `SpaDES` module consists of a single `.R` source file, containing the code for 1) the metadata describing the module and its data dependencies; 2) defining the event types described in the module; and 3) describing what happens during the processing of each event type.
...
- check module object dependency (simInit does this)
- check compatibility (simInit does a basic version of this)

It's up to the modeler (you!) to ensure the scientific validity of using different modules together.
...
```{r randomLandscapes, echo=TRUE, eval=FALSE, message=FALSE}
...
```{r fireSpread, echo=TRUE, eval=FALSE, message=FALSE}
...
```{r caribouMovement, echo=TRUE, eval=FALSE, message=FALSE}
